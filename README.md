...
- **上游模型目录** — 每个上游保留自己的文本和图片模型映射；路由表只决定请求走哪个上游，不会破坏上游映射关系。
- **小米视觉模型** — 图片请求可自动切换到 `mimo-v2-omni`，让 Claude Desktop 多模态请求走小米识图模型。
...
| `upstreams[].mappings` | 该上游自己的文本模型目录。多个上游可以保留同一个客户端模型名，各自映射到不同上游模型。 |
| `upstreams[].vision_mappings` | 该上游自己的图片模型目录。图片请求会优先使用这里的映射；不支持多模态的上游可留空。 |
